...
현재 버전은 **0.3.0 (창단)**. 2026년 7월 연고지·모기업·구장·1군 진입 방식을 골라 구단을 창단하고, 트라이아웃, 첫 신인 드래프트(우선지명), 퓨처스 시즌, 특별지명·FA 특례·외국인 계약을 거쳐 11구단 체제의 1군에 들어갑니다. 10개 구단 리그는 2015년부터 스스로 시즌을 치러 2026년 개막일에 도착해 있고, 구단 없이 리그만 관전할 수도 있습니다. 앞으로의 계획은 [`ROADMAP.md`](ROADMAP.md)에 있습니다.
...
npm run build && npm run test:browser   # 실제 Chromium: 창단부터 1군 진입까지, 4개 화면 크기
```
...
src/
  ...
  core/       시뮬레이션 버전
  league/     리그: 경기 엔진, 일정·순위·포스트시즌, AI 감독, 시즌·단계별 오프시즌, 과거 시뮬레이션, 창단(expansion.ts), 튜닝
